...
async function internal_email_add({ res, from, regreq }) {
	// Check type
	const type = getType(from);
	if (!type) {
		return helpers.formatApiResponse(400, res, Error(`Email ${from} is not allowed to register`));
	}
...
SendMessage.email_add = async function (req, res) {
	...
	await internal_email_add({ res, from, regreq });
};
