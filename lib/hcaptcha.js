...
hcaptcha.needCaptcha = async (req, res, next) => {
	function redirectCaptcha(req, res) {
		// return X-Redirect on /api calls, HTTP redirect otherwise
		if (req.originalUrl.startsWith('/api')) {
			res.setHeader('X-Redirect', captcha_path);
			res.status(200).send(captcha_path);
			// Redirect to path without '/api'
			req.session.returnTo = req.originalUrl.substring('/api'.length);
			return;
		}
