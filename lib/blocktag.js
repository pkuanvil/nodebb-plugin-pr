...
blockTag.pr_DefaultBlockTags = catchApiException(async (req, res) => {
	const defaultTags = await blockTag.getDefaultTags();
	return controllerHelpers.formatApiResponse(200, res, defaultTags);
});
...
blockTag.pr_userGlobalDefaults = async function (payload) {
	...
	// Escape comma so NodeBB translator won't treat string with comma as additional positional arguments: https://community.nodebb.org/post/59074
	const blockTagStr = default_block_tags || '';
	payload.defaultBlockTagsStr = blockTagStr.replace(/,/g, '&#44;');
	payload.defaultBlockTags = blockTagStr.toLowerCase().split(',');
};
